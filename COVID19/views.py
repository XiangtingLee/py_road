...
from COVID19.models import DXYData, DXYTimeLine
    ...
    maps.add_js_funcs(
        ...
        layui.use(['jquery'], function () {
            ...
            chart_tag.on("click", function(params){
                $.ajax({
                    url: "/COVID19/visualization/conversion/data/",
...
def visualization_view(request):
    ...
    resp = render(request, 'COVID19/visualization.html', data)
    resp.set_signed_cookie(key='sign', value=int(time.time()), salt=settings.SECRET_KEY,
                           path='/COVID19/visualization/')
    return resp
...
def timeline_view(request):
    resp = render(request, 'COVID19/timeline.html')
    # resp.set_signed_cookie(key='sign', value=int(time.time()), salt=settings.SECRET_KEY,
    #                        path='/COVID19/timeline/')
    return resp
